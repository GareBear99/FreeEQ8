...
public:
    ...
    // Smart EQ layer (v2.2.3) — wired to APVTS intent_mode parameter.
    // Called from editor's timerCallback() on the UI thread (lock-free).
    ResonanceDetector resonanceDetector;

    // Accessors for the UI layer (editor reads these, no lock needed)
    IntentMode getIntentMode() const noexcept
    {
        return static_cast<IntentMode>(
            (int)apvts.getRawParameterValue("intent_mode")->load());
    }
    bool isLinearPhaseActive() const noexcept
    {
        return apvts.getRawParameterValue("linear_phase")->load() > 0.5f;
    }
...
private:
    ...
    // Oversampling crossfade (v2.2.3 — fixes the "one-block latency blip")
    // When the user switches oversampling factor mid-playback, we crossfade
    // over kOsCrossfadeSamples between the old and new processed signal.
    // The crossfade buffer holds one block of the old path's output so we
    // can blend it with the new path's first block.
    static constexpr int kOsCrossfadeSamples = 128; // ~3ms at 44.1kHz — inaudible
    std::array<float, kOsCrossfadeSamples> osCrossfadeL {};
    std::array<float, kOsCrossfadeSamples> osCrossfadeR {};
    int osCrossfadeRemaining = 0;   // samples left in current crossfade
    bool osSwitchPending     = false;
