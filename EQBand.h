...
struct EQBand
{
    ...
    inline void maybeUpdateCoeffs(double sampleRate)
    {
        ...
        const bool smoothing = freqSm.isSmoothing() || qSm.isSmoothing() || gainSm.isSmoothing();
        ...
        {
            // Parameter smoothing only: update coefficients every coeffUpdateInterval
            // samples to amortise the cost. Smoothers must still be stepped every sample.
    ...
    inline void applySaturation(float& l, float& r) const
    {
        ...
        switch (satType)
        {
            ...
            case SaturationType::Transistor:
            {
                ...
                // Drive the signal up, hard-clip at ±1, then normalise back
                // to unity so that 0% drive is transparent.
