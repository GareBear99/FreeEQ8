...
\begin{abstract}
We present a production-grade parametric equalizer using the Simper State Variable Filter (SVF) topology for modulation-stable Dynamic EQ. Both SVF and RBJ use BLT with identical prewarping; SVF is chosen for automation stability and lower noise. The system consumes only 0.62\% of a single-core CPU budget at 44.1\,kHz. A lock-free SPSC triple-buffer isolates the audio thread from UI rendering, and a variable-cadence coefficient engine reduces Dynamic EQ overhead by 80\%. The implementation is available as FreeEQ8 (8-band, GPL-3.0) and ProEQ8 (24-band, commercial) from a single codebase. Live demonstration accompanies this paper.
\end{abstract}
