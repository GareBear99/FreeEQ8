...
void FreeEQ8AudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
    ...
    if (linearPhase)
    {
        // Linear phase and oversampling are mutually exclusive.
        // Oversampling operates on the biquad (minimum-phase) path only;
        // in linear phase mode the FIR kernel already encodes the full
        // composite magnitude response at the native sample rate, so
        // upsampling would only introduce unnecessary latency and aliasing
        // from the half-band filters. The UI greys out the oversampling
        // selector when linear phase is active; this branch enforces the
        // same separation in the audio path regardless of parameter state.
        //
        ...
        // Apply output gain separately (not baked into the magnitude kernel).
        for (int i = 0; i < n; ++i)
        {
            L[i] *= outputGain;
            R[i] *= outputGain;
