      ...
      levelMeter(p.meterPeakL, p.meterPeakR, p.meterRmsL, p.meterRmsR)
{
    ...
    // UI rendering optimisations (v2.2.3):
    // setOpaque(true) on the editor itself — the editor fills its entire
    // background solidly in paint(), so the host window compositor doesn't
    // need to read what's behind it. Eliminates one full-window alpha pass
    // on every frame repaint.
    setOpaque(true);
    ...
    // levelMeter content changes only when meter values update (~30 Hz);
    // setBufferedToImage(true) lets JUCE cache the last paint into an
    // image and only regenerate it when repaint() is explicitly called.
    levelMeter.setBufferedToImage(true);
...
void FreeEQ8AudioProcessorEditor::timerCallback()
{
    ...
    if (fifo.processIfReady())
    {
        ...
        // Smart EQ layer (v2.2.3): run ResonanceDetector on fresh spectrum.
        // This runs on the UI thread — completely allocation-free, no audio
        // thread involvement. Rate-limited to the 30 Hz spectrum update cadence.
        const IntentMode mode = proc.getIntentMode();
        proc.resonanceDetector.analyse(
            fifo.getMagnitudes(),
            fifo.getNumBins(),
            proc.getSampleRate() > 0 ? proc.getSampleRate() : 44100.0,
            mode);

        responseCurve.setSuggestions(proc.resonanceDetector.getSuggestions());

        // Pre-ring warning: DrumPunch + linear phase = transient smear risk
        responseCurve.setPreRingWarning(
            mode == IntentMode::DrumPunch && proc.isLinearPhaseActive());
    }
