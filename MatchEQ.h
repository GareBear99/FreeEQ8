...
public:
    ...
    void clear()
    {
        ...
        std::fill(correctionGain.begin(), correctionGain.end(), 1.0f);  // unity gain = no correction
...
private:
    ...
    // Pre-computed linear gain multipliers from correctionDb.
    // Computed once when the correction is finalised; eliminates numBins
    // pow(10) calls per audio block per channel on the hot path.
    ...
    void processAnalysisFrame()
    {
        ...
        if (frames >= analysisFramesNeeded)
        {
            ...
            // and pre-compute the equivalent linear gains so the hot-path
            // applyToChannel() never calls pow(10).
            for (int i = 0; i < numBins; ++i)
            {
    ...
    void applyToChannel(float* data, int n, std::array<float, fftSize>& overlap)
    {
        ...
        // Apply correction gains in frequency domain using pre-computed
        // linear multipliers (correctionGain[]). This avoids numBins pow(10)
        // calls per block per channel that the naive correctionDb path would incur.
